...
class _HomeScreenState extends State<HomeScreen> with SingleTickerProviderStateMixin {
  ...
  Future<void> _fetchSingle() async {
    ...
    try {
      var r = await _svc.getByDate(_ds(_selDate));
      if (r == null) {
        for (int o = 1; o <= 5; o++) {
          final nd = _selDate.subtract(Duration(days: o));
          r = await _svc.getByDate(_ds(nd));
          if (r != null) {
            setState(() {
  ...
  Future<void> _fetchHist() async {
    ...
    try {
      final r = await _svc.getByRange(_ds(_from), _ds(_to));
      setState(() {
  ...
  Future<void> _exportExcel() async {
    ...
    final rows = <List<dynamic>>[
      ['Дата', 'AM ($currency)', 'PM ($currency)'],
    ];
    for (final r in _histRes) {
      rows.add([
        r.date,
        r.am?.priceForCurrency(_cur) ?? '',
        r.pm?.priceForCurrency(_cur) ?? '',
      ]);
    }
    ...
    final file = File('${dir.path}/lbma_gold_${_ds(_from)}_${_ds(_to)}.csv');
    await file.writeAsString(bom + csv);

    await Share.shareXFiles(
      [XFile(file.path)],
      text: 'LBMA Gold Price $currency (${LFC.fmtDate(_ds(_from))} — ${LFC.fmtDate(_ds(_to))})',
    );
  }

  // ==================== WIDGETS ====================

  // Red gradient header (Liverpool style)
                ...
                child: Row(
                  children: [
                    ...
                    const Column(
                      ...
                      children: [
                        Text('LBMA GOLD PRICE', style: TextStyle(
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 12),
                  // AM row
                  _priceRow('AM', '10:30 London', aP, true),
                  Container(height: 1, color: LFC.border),
                  // PM row
                  _priceRow('PM', '15:00 London', pP, false),
                  if (diff != null) ...[
                  ...
                  const SizedBox(height: 12),
                  const Text('Цена за 1 тройскую унцию (31.1 г) · LBMA',
                    style: TextStyle(fontSize: 11, color: LFC.muted)),
                ],
              ),
            ),
          ],
      ...
      children: [
        ...
        if (_histRes.isNotEmpty) ...[
          ...
          Padding(
            ...
            child: Text(
              '${_histRes.length} записей · ${LFC.currencies[_cur]} за тройскую унцию · LBMA',
        ...
        child: Column(children: [
          ...
          Container(
            ...
            child: const Row(children: [
              Expanded(flex: 3, child: Text('ДАТА', style: TextStyle(
                fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white70, letterSpacing: 1,
              ))),
              Expanded(flex: 2, child: Text('AM', textAlign: TextAlign.right, style: TextStyle(
                fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white70, letterSpacing: 1,
              ))),
              Expanded(flex: 2, child: Text('PM', textAlign: TextAlign.right, style: TextStyle(
                fontSize: 12, fontWeight: FontWeight.w700, color: Colors.white70, letterSpacing: 1,
              ))),
            ]),
          ),
              ...
              itemBuilder: (ctx, idx) {
                ...
                return Container(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(flex: 2, child: Text(LFC.fmtPrice(p, _cur),
                      textAlign: TextAlign.right,
                      style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: LFC.text))),
                  ]),
                );
